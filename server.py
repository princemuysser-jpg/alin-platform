...
import os, sqlite3, uuid, hashlib, hmac, time, json
...
def execsql(q,args=()):
    with con() as c: c.execute(q,args); c.commit()
def audit(kind,text):
    execsql("INSERT INTO audit(id,kind,text,created_at) VALUES(?,?,?,datetime('now'))",(uid("A"),kind,text))
def init():
    UPLOADS.mkdir(parents=True,exist_ok=True); DB.parent.mkdir(parents=True,exist_ok=True)
    with con() as c:
        c.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS booklets(id TEXT PRIMARY KEY,title TEXT NOT NULL,teacher_id TEXT NOT NULL,subject TEXT,grade TEXT,price INTEGER NOT NULL,cover_path TEXT,teacher_image_path TEXT,file_path TEXT,file_name TEXT,status TEXT DEFAULT 'published',created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(teacher_id) REFERENCES accounts(id));
        ...
        if not c.execute("SELECT 1 FROM accounts LIMIT 1").fetchone():
            c.execute("INSERT INTO accounts(id,role,name,username,password_hash,area,landmark,status) VALUES(?,?,?,?,?,?,?,?)",("T1","teacher","ميسر صلاح الدين","0770",generate_password_hash("1234"),"كركوك","","active"))
            c.execute("INSERT INTO accounts(id,role,name,username,password_hash,area,landmark,status) VALUES(?,?,?,?,?,?,?,?)",("L1","library","مكتبة آلين","LIB1",generate_password_hash("1234"),"كركوك","قرب المركز","active"))
            c.execute("INSERT INTO booklets(id,title,teacher_id,subject,grade,price,status) VALUES(?,?,?,?,?,?,?)",("B1","ملزمة الفيزياء","T1","الفيزياء","السادس الإعدادي",10000,"published"))
            c.execute("INSERT INTO banners VALUES(?,?,?,1)",("AD1","الأستاذ ميسر صلاح الدين انضم إلى منصة آلين","ملازمه متوفرة الآن"))
        if not c.execute("SELECT 1 FROM categories LIMIT 1").fetchone():
            for cid, typ, name in [
                ("C1", "booklet", "السادس الإعدادي"),
                ("C2", "stationery", "دفاتر"),
                ("C3", "stationery", "أقلام"),
                ("C4", "gift", "هدايا"),
            ]:
                c.execute("INSERT INTO categories(id,type,name,status) VALUES(?,?,?,'active')",(cid,typ,name))
        c.commit()
init()
...
def add_account():
    ...
    except sqlite3.IntegrityError: return jsonify(error="اليوزر مستخدم"),409
...
def save_file(field, folder, allowed):
    f=request.files.get(field)
    if not f or not f.filename: return None,None
    ext=Path(f.filename).suffix.lower()
    if ext not in allowed: raise ValueError("امتداد الملف غير مسموح")
    name=uuid.uuid4().hex+ext; p=UPLOADS/folder; p.mkdir(exist_ok=True)
    f.save(p/name); return f"{folder}/{name}",secure_filename(f.filename)

@app.post("/api/booklets")
def add_booklet():
    try:
        cover,_=save_file("cover","covers",{".png",".jpg",".jpeg",".webp"})
        ti,_=save_file("teacherImage","teachers",{".png",".jpg",".jpeg",".webp"})
        fp,fn=save_file("bookletFile","booklets",{".pdf"})
        form=request.form
...
def create_order():
    ...
    # payment_pending: no card/CVV is accepted or stored by Alin.
...
def mock_confirm(oid):
    # DEVELOPMENT ONLY. Replace with verified provider webhook in production.
    ...
    with con() as c:
        c.execute("UPDATE orders SET status='paid',payment_status='paid',payment_ref=? WHERE id=?",("DEV-"+uid("PAY"),oid))
        c.execute("INSERT INTO permits(id,order_id,booklet_id,library_id,qty,used,status) VALUES(?,?,?,?,?,0,'active')",(uid("P"),oid,b["id"],o["library_id"],o["qty"]))
        c.execute("INSERT INTO ledger(id,order_id,alin,teacher,teacher_id,library,library_id,settlement_status) VALUES(?,?,?,?,?,?,?,'unsettled')",(uid("LG"),oid,alin,teacher,b["teacher_id"],library,o["library_id"]))
        c.commit()
...
def use_permit(pid):
    with con() as c:
        p=c.execute("SELECT * FROM permits WHERE id=?",(pid,)).fetchone()
        ...
        c.execute("UPDATE permits SET used=?,status=? WHERE id=?",(used,status,pid)); c.commit()
...
def add_product():
    try:
        img,_=save_file("image","products",{".png",".jpg",".jpeg",".webp"})
        f=request.form
        pid=uid("PR")
...
@app.get("/api/backup")
def db_backup():
    return send_from_directory(DB.parent,DB.name,as_attachment=True,download_name="alin_v12_backup.db")

...
const version=fs.readFileSync(path.join(root,'VERSION'),'utf8').trim();if(version!=='3.0.0-production-unified')failures.push(`version:${version}`);
for(const name of ['store-desktop.html','store-mobile.html']){
  const html=fs.readFileSync(path.join(root,name),'utf8');
  if(!html.includes('v3.0.0'))failures.push(`html-version:${name}`);

...
renderLibrary=function(){if(!window.libraryStats)return;const id=current?.role==='library'?current.id:'';const orders=db.orders.filter(x=>x.library_id===id),led=db.ledger.filter(x=>x.library_id===id),notices=v19Notifications.filter(n=>n.status==='active'&&((n.target_role||n.audience)==='all'||(n.target_role||n.audience)==='library'));libraryStats.innerHTML=`<div><b>طلبات المكتبة</b><span>${orders.length}</span></div><div><b>الجديدة</b><span>${orders.filter(x=>x.status==='new').length}</span></div><div><b>الجاهزة</b><span>${orders.filter(x=>x.status==='ready').length}</span></div><div><b>المستحقات</b><span>${money(led.reduce((a,x)=>a+(+x.library||0),0))} د.ع</span></div>`;libraryPermits.innerHTML=notices.map(n=>`<div class="notice"><b>${esc(n.title)}</b><div>${esc(n.message||n.text||'')}</div></div>`).join('')+orders.map(x=>`<div class="row"><div><b>${esc(x.order_number||x.id)} — ${esc(x.title)}</b><small>${esc(x.student_name)} • ${esc(x.student_phone||'')} • ×${x.qty}</small></div><div class="row-actions"><button onclick="libraryOrderStatus('${x.id}','processing')">استلام</button><button onclick="libraryOrderStatus('${x.id}','ready')">جاهز</button><button onclick="libraryOrderStatus('${x.id}','completed')">تسليم</button></div></div>`).join('')||emptyState('لا توجد طلبات');libraryHistory.innerHTML=orders.filter(x=>x.status==='completed').map(x=>`<div class="row"><b>${esc(x.order_number||x.id)}</b><span>${money(x.total)} د.ع</span></div>`).join('')||emptyState('لا يوجد سجل تسليم');}
...
renderLibrary=function(){
  ...
  const orders=db.orders.filter(x=>x.library_id===id), led=db.ledger.filter(x=>x.library_id===id), notices=v19Notifications.filter(n=>n.status==='active'&&((n.target_role||n.audience)==='all'||(n.target_role||n.audience)==='library'));
  ...
  libraryPermits.innerHTML=statusBox+notices.map(n=>`<div class="notice"><b>${esc(n.title)}</b><div>${esc(n.message||n.text||'')}</div></div>`).join('')+orders.map(x=>{
...
renderLibrary=function(){
  ...
  const orders=(db.orders||[]).filter(x=>x.library_id===id),led=(db.ledger||[]).filter(x=>x.library_id===id),notices=(v19Notifications||[]).filter(n=>n.status==='active'&&((n.target_role||n.audience)==='all'||(n.target_role||n.audience)==='library'));
  libraryStats.innerHTML=`<div><b>طلبات المكتبة</b><span>${orders.length}</span></div><div><b>الجديدة</b><span>${orders.filter(x=>x.status==='new').length}</span></div><div><b>الجاهزة</b><span>${orders.filter(x=>x.status==='ready').length}</span></div><div><b>المستحقات</b><span>${money(led.reduce((a,x)=>a+(+x.library||0),0))} د.ع</span></div>`;
  libraryPermits.innerHTML=notices.map(n=>`<div class="notice"><b>${esc(n.title)}</b><div>${esc(n.message||n.text||'')}</div></div>`).join('')+orders.map(x=>`<div class="row"><div><b>${esc(x.order_number||x.id)} — ${esc(x.title)}</b><small>${esc(x.student_name)} • ${esc(x.student_phone||'')} • ×${x.qty} • ${esc(x.status||'')}</small></div><div class="row-actions">${x.kind==='booklet'?`<button class="library-pdf-btn" onclick="openLibraryBookletPdf('${x.id}')">عرض PDF</button>`:''}<button onclick="libraryOrderStatus('${x.id}','processing')">استلام</button><button onclick="libraryOrderStatus('${x.id}','ready')">جاهز</button><button onclick="libraryOrderStatus('${x.id}','completed')">تسليم</button><button class="secondary" onclick="printOrderReceipt('${x.id}')">وصل</button></div></div>`).join('')||emptyState('لا توجد طلبات');
  libraryHistory.innerHTML=orders.filter(x=>x.status==='completed').map(x=>`<div class="row"><b>${esc(x.order_number||x.id)}</b><span>${money(x.total)} د.ع</span></div>`).join('')||emptyState('لا يوجد سجل تسليم');
};
...
window.renderProductsAdmin=renderProductsAdmin=function(){oldProducts();adminContent.insertAdjacentHTML('afterbegin','<div class="alin-v84-admin-shortcuts"><button onclick="alinV84ControlCenter()">⚙ مركز تشغيل المتجر</button><button onclick="alinV84Reports()">📊 التقارير</button></div>'); const stock=(db.products||[]).filter(p=>+p.stock<=+(p.low_stock_limit||db.settings?.low_stock_default||5));if(stock.length)adminContent.insertAdjacentHTML('afterbegin',`<div class="alin-v84-lowstock">⚠ تنبيه مخزون: ${stock.map(p=>esc(p.name||p.title)).join('، ')}</div>`)};
...
setInterval(alinV84RefreshBadges,5000);
})();
...
(function(){
  ...
  setInterval(()=>{alinV85Badges()},5000);
...
(function(){
  ...
  function ensureMobileShell(){
    ...
    // Remove accidental duplicate mobile shells.
    document.querySelectorAll('.alin-v100-mobile-header').forEach((el,i)=>{if(i>0)el.remove()});
    document.querySelectorAll('.alin-v100-mobile-nav').forEach((el,i)=>{if(i>0)el.remove()});

    if(!document.querySelector('.alin-v100-mobile-header')){
      const header=document.createElement('header');
      header.className='alin-v100-mobile-header';
      header.innerHTML=`
        <div class="alin-v100-brand">
          <div class="alin-v100-logo">آ</div>
          <div><b>منصة آلين</b><small>الملازم والقرطاسية والهدايا</small></div>
        </div>
        <div class="alin-v100-header-actions">
          <button class="alin-v100-icon-btn" onclick="document.querySelector('.alin-v94-notification-button,.alin-v78-notify-btn')?.click()" aria-label="الإشعارات">🔔</button>
          <button class="alin-v100-icon-btn" onclick="alinV100ToggleMenu()" aria-label="القائمة">☰</button>
        </div>`;
      store.prepend(header);
    }

    if(!document.querySelector('.alin-v100-mobile-nav')){
      const nav=document.createElement('nav');
      nav.className='alin-v100-mobile-nav';
      nav.innerHTML=`
        <button class="active" onclick="alinV100Go('home',this)"><span>⌂</span><small>الرئيسية</small></button>
        <button onclick="alinV100Go('store',this)"><span>▦</span><small>المتجر</small></button>
        <button onclick="alinV100Go('orders',this)"><span>◷</span><small>الطلبات</small></button>
        <button onclick="alinV100Go('favorites',this)"><span>♥</span><small>المفضلة</small></button>
        <button onclick="alinV100Go('account',this)"><span>◉</span><small>الحساب</small></button>`;
      document.body.appendChild(nav);
    }

    if(!document.querySelector('.alin-v100-side-menu')){
      const menu=document.createElement('aside');
      menu.className='alin-v100-side-menu';
      menu.innerHTML=`
        <div class="alin-v100-menu-card">
          <button class="alin-v100-menu-close" onclick="alinV100ToggleMenu()">×</button>
          <h3>منصة آلين</h3>
          <button onclick="alinV100Go('home');alinV100ToggleMenu()">الرئيسية</button>
          <button onclick="alinV100Go('store');alinV100ToggleMenu()">المتجر</button>
          <button onclick="alinV100Go('orders');alinV100ToggleMenu()">طلباتي</button>
          <button onclick="alinV100Go('favorites');alinV100ToggleMenu()">المفضلة</button>
          <button onclick="alinV100Go('account');alinV100ToggleMenu()">الحساب</button>
        </div>`;
  ...
  function decorateHero(){
    const store=document.getElementById('storePage');
    if(!store)return;
    let hero=store.querySelector('.alin-v100-hero');
    if(!hero){
      hero=document.createElement('section');
      hero.className='alin-v100-hero';
      hero.innerHTML=`
        <div>
          <span>منصة الطالب</span>
          <h1>كل ما تحتاجه للدراسة<br>بمكان واحد</h1>
          <p>اختر ملزمتك أو قرطاسيتك واطلبها بسهولة.</p>
          <div><button onclick="alinV100Go('store')">تصفح المتجر</button><button class="secondary" onclick="alinV100Go('orders')">تتبع الطلب</button></div>
        </div>
        <div class="alin-v100-hero-mark">ALIN<small>منصة آلين</small></div>`;
      const anchor=store.querySelector('#bannerBox')||store.children[1];
  ...
  function decorateCategories(){
    const store=document.getElementById('storePage');
    if(!store)return;
    let cats=store.querySelector('.alin-v100-categories');
    if(!cats){
      cats=document.createElement('section');
      cats.className='alin-v100-categories';
      cats.innerHTML=`
        <button onclick="setStoreType('booklet')"><span>📚</span><small>ملازم</small></button>
        <button onclick="setStoreType('stationery')"><span>✏️</span><small>قرطاسية</small></button>
        <button onclick="setStoreType('gift')"><span>🎁</span><small>هدايا</small></button>
        <button onclick="document.getElementById('bannerBox')?.scrollIntoView({behavior:'smooth'})"><span>🔥</span><small>عروض</small></button>`;
      const hero=store.querySelector('.alin-v100-hero');
      hero?.insertAdjacentElement('afterend',cats);
    }
  }
  ...
  function decorateSections(){
    ...
    if(parent&&!parent.querySelector('.alin-v100-section-title')){
      grid.insertAdjacentHTML('beforebegin','<div class="alin-v100-section-title"><div><small>مختارات آلين</small><h2>الأكثر طلباً</h2></div><button onclick="document.getElementById(\'storeGrid\').scrollIntoView({behavior:\'smooth\'})">عرض الكل</button></div>');
    }
  }

  window.alinV100ToggleMenu=function(){
    document.querySelector('.alin-v100-side-menu')?.classList.toggle('open');
  };

  window.alinV100Go=function(view,button){
    document.querySelectorAll('.alin-v100-mobile-nav button').forEach(b=>b.classList.remove('active'));
    ...
    if(view==='home'){
      ...
      if(typeof trackOrder==='function'){
        document.getElementById('trackOrderBox')?.scrollIntoView({behavior:'smooth'});
      }else{
        alert('قسم الطلبات غير متاح حالياً');
      }
    }else if(view==='favorites'){
      if(typeof alinV85Favorites==='function')alinV85Favorites();
      else if(typeof alinV84ShowFavs==='function')alinV84ShowFavs();
      else alert('لا توجد مفضلة محفوظة');
    }else if(view==='account'){
      const login=document.getElementById('studentAuthModal');
      if(login)login.classList.remove('hidden');
      else alert('الحساب غير متاح حالياً');
    }
  };
  ...
  function decorateCards(){
    if(!isMobile())return;
    document.querySelectorAll('#storeGrid .alin-v76-card').forEach(card=>{
      card.classList.add('alin-v100-product-card');
      const body=card.querySelector('.alin-v76-body');
      if(body&&!body.querySelector('.alin-v100-card-meta')){
        const stock=body.querySelector('.alin-v76-stock');
        if(stock)stock.classList.add('alin-v100-card-meta');
      }
    });
  }
  ...
  if(typeof previousRenderStore==='function'){
    window.renderStore=renderStore=function(){
      const result=previousRenderStore.apply(this,arguments);
      setTimeout(()=>{ensureMobileShell();decorateCards()},0);
      return result;
    };
  }

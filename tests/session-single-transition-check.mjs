...
if(platform.includes('function openPage(')||platform.includes('oldOpenPage')||platform.includes('_openPageV'))failures.push('platform:legacy-navigation-remains');
if(!navigation.includes('function openPage(page,options={})')||!navigation.includes("if(options.render!==false&&typeof window.renderAll==='function')window.renderAll()"))failures.push('navigation:render-control');
...
if(!navigation.includes('logoutPromise')||!auth.includes('explicitSignOut')||!auth.includes("event==='SIGNED_OUT'&&!explicitSignOut"))failures.push('logout:single-transition-guard');
if(/window\.(?:doLogin|logout)\s*=/.test(auth))failures.push('auth:handler-replacement');
if(/oldOpen|oldLogout|oldLogin/.test(security))failures.push('security:handler-wrapper');

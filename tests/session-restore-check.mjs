...
const end=source.indexOf('backend readiness diagnostics',start);
const blockEnd=source.lastIndexOf('/*',end);
...
let bootRemoved=0,opened='',openOptions=null,loaded=0,libraryShown=0,restoredEvent=0;
...
const window={
  ...
  current:null,pendingRole:'',load:async()=>{loaded++},openPage:(page,options)=>{opened=page;openOptions=options},logout(){},
...
assert.ok(loaded>=1);
assert.equal(libraryShown,1);
assert.equal(openOptions?.render,false);

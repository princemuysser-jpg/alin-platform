...
(function(){
  ...
  window.applyBrand = function(){
    ...
    try{
      ...
      document.title=(db?.settings?.platform_name||'منصة آلين')+' V28.2';
    }catch(e){}
  };
})();
...
openPage=function(page){_openPageV29(page);setTimeout(()=>{document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V29 Delivery + COD'); document.title='منصة آلين V29';},80)};
...
(function(){
  ...
  window.uploadBooklet=async function(){
    try{const f=new FormData(bookForm);const cover=await uploadFile('covers',f.get('cover'));const ti=await uploadFile('teachers',f.get('teacherImage'));const pdfFile=f.get('bookletFile');if(!pdfFile||!pdfFile.name)throw new Error('ملف PDF مطلوب');const fp=await uploadFile('booklets',pdfFile);await insert('booklets',{id:uid('B'),title:f.get('title'),teacher_id:f.get('teacherId'),subject:f.get('subject'),grade:f.get('grade'),edition:f.get('edition')||'نسخة 2027',price:+f.get('price'),cover_path:cover,teacher_image_path:ti,file_path:fp,file_name:pdfFile.name,status:'published'});await audit('booklet','نشر ملزمة '+f.get('title'));await load();renderBookletsAdmin();toast('تم نشر الملزمة');}catch(e){alert(e.message);}
  };
  ...
  window.openPage=function(page){oldOpenPage(page);setTimeout(()=>{document.title='منصة آلين V31';document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V31');},50)};
})();
...
(function(){
  ...
  const oldOpenPage=window.openPage;
  window.openPage=function(page){oldOpenPage(page);setTimeout(()=>{renderBottomInfo();document.title='منصة آلين V32';document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V32');},80)};
})();

...
(function(){
  ...
  function allSettlements(){
    ...
    return unique([
      ...
      ...arr(data.settlements)
    ],row=>row.id||row.receipt_number||row.voucher_number||`${row.created_at}-${row.amount}-${row.party_id||row.account_id||''}`);
  }
  ...
  function settlementNumber(row){return String(row.receipt_number||row.voucher_number||row.id||'تسوية')}
  function orderKey(row){return encodeURIComponent(String(row.id||row.order_id||row.order_number||row.tracking_code||''))}
  function settlementKey(row){return encodeURIComponent(String(row.id||row.receipt_number||row.voucher_number||''))}
  ...
  function findSettlement(key,role){
    const value=decodeURIComponent(String(key||''));
    return scopedSettlements(role).find(row=>[row.id,row.receipt_number,row.voucher_number].some(item=>same(item,value)))||null;
  }
  ...
  function settlementRow(row,role){
    ...
    return `<article class="alin415r-row" data-alin415r-kind="settlement" data-alin415r-status="${esc(status)}" data-alin415r-search="${esc(search)}">
      ...
      <time>${esc(dateTime(row.created_at||row.updated_at))}</time>
      <strong>${money(row.amount)} د.ع</strong>
  ...
  function settlementReceipt(row,role){
    ...
    return `<article class="alin415r-paper" dir="rtl" data-alin415r-printable>
      ...
      <div class="alin415r-paper-meta"><div><small>التاريخ</small><b>${esc(dateTime(row.created_at||row.updated_at))}</b></div><div><small>نوع الحساب</small><b>${esc(roleName(actual))}</b></div><div><small>الحالة</small><b>${esc(receiptStatus(row,'settlement'))}</b></div></div>
      <section class="alin415r-paper-section"><h3>معلومات التسوية</h3><div class="alin415r-student"><div><small>اسم الحساب</small><b>${esc(settlementPartyName(row,role))}</b></div><div><small>طريقة التسوية</small><b>${esc(row.payment_method||'نقداً')}</b></div><div><small>رقم الوصل</small><b dir="ltr">${esc(number)}</b></div></div></section>
      <section class="alin415r-settlement-amount"><small>المبلغ المسدد</small><strong>${money(row.amount)} د.ع</strong></section>
  ...
  function install(){
    ...
    window.addEventListener('alin:data-refreshed',()=>{
      const role=document.body.dataset.alin415ReceiptsRole;
      if(role&&hostFor(role)&&!hostFor(role).closest('.hidden'))renderCenter(role);
    });
  }

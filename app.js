...
function renderSettingsAdmin(){ adminContent.innerHTML=`<h2>إعدادات المنصة</h2><p>الاتصال بقاعدة البيانات مثبت مباشرة داخل النظام.</p><div class="form-grid"><input id="platformNameInput" value="آلين" placeholder="اسم المنصة"><input id="platformPhoneInput" value="" placeholder="رقم الهاتف"><button onclick="saveSystemSettings()">حفظ</button></div><div id="systemMsg"></div>`; }
...
function applyBrand(){ const n=db.settings.platform_name||'منصة آلين'; document.title=n+' V19.2'; document.querySelectorAll('.brand b').forEach(x=>x.textContent=n.replace('منصة ','')); const hero=document.querySelector('.hero'); if(hero)hero.innerHTML=`<h2>${esc(db.settings.hero_title||n)}</h2><p>${esc(db.settings.hero_text||'')}</p>`; }
...
applyBrand = function(){
  v21OldApplyBrand();
  document.title=(db.settings.platform_name||'منصة آلين')+' V22';
  const badge=document.querySelector('.version-badge'); if(badge) badge.textContent='V23 Malzama';
};
...
load = async function(){
  await v22Load();
  if(sb){
    try{ db.teacherRequests = await query('teacher_requests'); }catch(e){ db.teacherRequests=[]; console.warn('نفذ supabase_v22.sql لطلبات المدرسين', e.message); }
...
async function sendTeacherBookRequest(){
  try{
    ...
    const path=sourceFile&&sourceFile.name?await uploadFile('teacher-requests',sourceFile):'';
...
async function saveTeacherProfile(){
  try{
    const avatar=tpAvatar.files&&tpAvatar.files[0]?await uploadFile('teachers',tpAvatar.files[0]):(teacherData().teacher.avatar_path||'');
...
async function recordPayout(role,id){
  ...
  const method=prompt('طريقة الدفع: نقدي / تحويل / زين كاش / آسيا حوالة','نقدي')||'نقدي', note=prompt('ملاحظة الدفعة','')||'';
...
function printLibrarySettlement(id){
  ...
  const html=`<div dir="rtl" style="font-family:Arial;padding:24px"><h2>سند قبض تسوية مكتبة</h2><p><b>رقم السند:</b> ${esc(s.receipt_number)}</p><p><b>المكتبة:</b> ${esc(lib.name||'')}</p><p><b>المبلغ:</b> ${money(s.amount)} د.ع</p><p><b>طريقة الاستلام:</b> ${esc(s.payment_method||'')}</p><p><b>الفترة:</b> ${esc(s.period_from||'-')} إلى ${esc(s.period_to||'-')}</p><p><b>ملاحظة:</b> ${esc(s.note||'')}</p><hr><p>توقيع الإدارة: ____________</p><p>توقيع المكتبة: ____________</p></div>`;
...
try{document.title=(db?.settings?.platform_name||'منصة آلين')+' V27'}catch(e){}
...
adminStatsRender=function(){
  ...
  document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V27 UI Refresh');
};
const _openPageV27=openPage;
openPage=function(page){
  _openPageV27(page);
  setTimeout(()=>{document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V27 UI Refresh');},50);
};

/* ================= ALIN V28 BRAND MANAGER ================= */
window.ALIN_VERSION='V28 Brand Manager';
try{document.title=(db?.settings?.platform_name||'منصة آلين')+' V28'}catch(e){}
...
function applyBrandV28(){
  ...
  document.querySelectorAll('.logo').forEach(x=>setLogoNode(x,logoUrl,'آ'));
...
openPage=function(page){_openPageV28(page);setTimeout(applyBrandV28,50);setTimeout(()=>{document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V28 Brand Manager');},60)};
const _adminStatsRenderV28=adminStatsRender;
adminStatsRender=function(){_adminStatsRenderV28();document.querySelectorAll('.version-badge').forEach(x=>x.textContent='V28 Brand Manager')};

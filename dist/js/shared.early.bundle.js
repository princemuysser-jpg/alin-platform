// Alin shared early bundle v2.0.5
...
async function ensureStorageReady(){
  ...
  try{
    ...
    if(error){
      ...
      if(m.includes('bucket') || m.includes('not found')) throw new Error('مجلد التخزين alin-files غير موجود. نفّذ ملف RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql مرة واحدة.');
      if(m.includes('policy') || m.includes('permission') || m.includes('row-level')) throw new Error('صلاحيات التخزين ناقصة. نفّذ ملف RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql مرة واحدة.');
    }
  }catch(e){
    if(String(e.message||'').includes('RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql')) throw e;
  }
}
...
async function uploadFile(bucketPath, file, opts={}){
  ...
  if(error){
    const m=String(error.message||'').toLowerCase();
    if(m.includes('bucket') || m.includes('not found')) throw new Error('مجلد التخزين alin-files غير موجود. نفّذ ملف RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql مرة واحدة.');
    if(m.includes('policy') || m.includes('permission') || m.includes('row-level')) throw new Error('صلاحيات الرفع ناقصة. نفّذ ملف RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql مرة واحدة.');
  ...
  try{
    ...
    if(!ok) throw new Error('الملف رُفع لكن الرابط غير عام. نفّذ ملف RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql.');
  }catch(e){
    if(String(e.message||'').includes('RUN_ON_SUPABASE_v2_0_5_COMPLETE.sql')) throw e;
  }

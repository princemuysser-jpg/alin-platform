...
-- ============================================================
-- ============================================================
-- 5A) تعيين الطلب من الإدارة عبر مسار واحد آمن
-- ============================================================
...
declare
  ...
  v_source text;
  v_current_courier text;
  v_order_area text;
  v_has_declared_area boolean:=false;
  v_area_match boolean:=false;
begin
  if not public.alin_is_admin() then
    raise exception 'تعيين الطلب متاح للإدارة فقط';
  end if;
  ...
  v_current_courier:=coalesce(nullif(o.courier_id::text,''),nullif(o.delegate_id::text,''));

  if v_source in ('completed','delivered','cancelled','rejected') then
    raise exception 'لا يمكن تعيين طلب مكتمل أو ملغي';
  end if;

  if v_source in ('accepted','picked_up','out_for_delivery','out_delivery','processing')
     and coalesce(v_current_courier,'')<>coalesce(p_courier_id,'') then
    raise exception 'لا يمكن تغيير المندوب بعد قبول الطلب أو استلامه';
  end if;

  if nullif(btrim(coalesce(p_courier_id,'')),'') is not null then
    select * into a
    from public.accounts
    where id::text=p_courier_id
      and lower(coalesce(role,''))='courier'
      and lower(coalesce(status,'active'))='active'
      and deleted_at is null;
    if not found then raise exception 'المندوب غير موجود أو غير فعال'; end if;

    if to_regclass('public.couriers') is not null then
      execute 'select to_jsonb(c) from public.couriers c where c.id::text=$1'
      into c using p_courier_id;
    end if;

    v_order_area:=lower(regexp_replace(btrim(coalesce(o.delivery_area,'')),'[[:space:]-–—_]+','','g'));
    if v_order_area<>'' then
      if lower(regexp_replace(btrim(coalesce(a.area,'')),'[[:space:]-–—_]+','','g'))<>'' then
        v_has_declared_area:=true;
        v_area_match:=lower(regexp_replace(btrim(coalesce(a.area,'')),'[[:space:]-–—_]+','','g'))=v_order_area;
      end if;

      if c is not null and lower(regexp_replace(btrim(coalesce(c->>'area','')),'[[:space:]-–—_]+','','g'))<>'' then
        v_has_declared_area:=true;
        v_area_match:=v_area_match or lower(regexp_replace(btrim(coalesce(c->>'area','')),'[[:space:]-–—_]+','','g'))=v_order_area;
      end if;

      if c is not null and jsonb_typeof(c->'areas')='array' and jsonb_array_length(c->'areas')>0 then
        v_has_declared_area:=true;
        select v_area_match or exists(
          select 1 from jsonb_array_elements_text(c->'areas') x(value)
          where lower(regexp_replace(btrim(value),'[[:space:]-–—_]+','','g'))=v_order_area
        ) into v_area_match;
      end if;

      if v_has_declared_area and not v_area_match then
        raise exception 'المندوب غير مرتبط بمنطقة الطلب';
      end if;
    end if;
    ...
    update public.orders set
      ...
      status_history=coalesce(status_history,'[]'::jsonb)||jsonb_build_array(jsonb_build_object(
        'status','assigned','at',v_now,'by',public.alin_current_account_id(),
        'role','admin','courier_id',p_courier_id
      )),
      updated_at=v_now
    where id::text=p_order_id
    ...
    if nullif(btrim(coalesce(p_library_id,'')),'') is not null then
      select * into a
      from public.accounts
      where id::text=p_library_id
        and lower(coalesce(role,''))='library'
        and lower(coalesce(status,'active'))='active'
        and deleted_at is null;
      if not found then raise exception 'المكتبة غير موجودة أو غير فعالة'; end if;
    end if;
    ...
    update public.orders set
      courier_id=null,
      delegate_id=null,
      library_id=nullif(btrim(coalesce(p_library_id,'')),''),
      pickup_library_id=nullif(btrim(coalesce(p_library_id,'')),''),
      ...
      delivery_note=null,
      status_history=coalesce(status_history,'[]'::jsonb)||jsonb_build_array(jsonb_build_object(
        'status','pending_admin','at',v_now,'by',public.alin_current_account_id(),
        'role','admin','library_id',nullif(btrim(coalesce(p_library_id,'')),'')
      )),
      updated_at=v_now
    where id::text=p_order_id
    returning * into v_updated;
  ...
  return jsonb_build_object('ok',true,'order',to_jsonb(v_updated));
end
$$;
revoke all on function public.alin_admin_assign_order(text,text,text) from public,anon;

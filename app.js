...
function mediaUrl(path){
  ...
  // يدعم الروابط القديمة التي كانت مخزنة كباكيتات منفصلة مثل booklets/covers/teachers
  try{
    if(/^https?:\/\//i.test(raw)){
      const u=new URL(raw);
      ...
      if(i>=0){
        const tail=decodeURIComponent(u.pathname.slice(i+marker.length));
        const parts=tail.split('/').filter(Boolean);
        const oldBucket=parts.shift()||'';
        if(oldBucket && oldBucket!=='alin-files'){
          const folderMap={booklets:'booklets',covers:'covers',teachers:'teachers',brand:'brand',products:'products'};
          raw=(folderMap[oldBucket]||oldBucket)+'/'+parts.join('/');
        }else if(oldBucket==='alin-files'){
          raw=parts.join('/');
        }else{
          return path;
        }

...
(function(){
  ...
  window.alinV161AddArea=async function(){const name=(prompt('اسم المنطقة الجديدة')||'').trim();if(!name)return;try{await insert('delivery_areas',{id:uidV('A'),name,city:'كركوك',active:true,sort_order:areaRows().length+1});if(typeof load==='function')await load();renderCourierAreasAdmin()}catch(e){alert('نفّذ ملف SQL الخاص بـ V161 أولاً، أو تحقق من صلاحيات جدول المناطق')}};
  ...
  window.alinV161DeleteArea=async function(id,name){if((window.couriers||[]).some(c=>areaNames(c).includes(name)))return alert('لا يمكن حذف منطقة مرتبطة بمندوب. أزل الربط أولاً.');if(!confirm('حذف منطقة '+name+'؟'))return;try{await update('delivery_areas',{active:false},{id});if(typeof load==='function')await load();renderCourierAreasAdmin()}catch(e){alert('تعذر حذف المنطقة')}};

...
async function uploadBooklet(){ try{ const f=new FormData(bookForm); const cover=await uploadFileV52('covers',f.get('cover'),{type:'image'}); const ti=await uploadFileV52('teachers',f.get('teacherImage'),{type:'image'}); const pdfFile=f.get('bookletFile'); if(!pdfFile||!pdfFile.name)throw new Error('ملف PDF مطلوب'); const fp=await uploadFileV52('booklets',pdfFile,{required:true,type:'pdf'}); await insert('booklets',{id:uid('B'),title:f.get('title'),teacher_id:f.get('teacherId'),subject:f.get('subject'),grade:f.get('grade'),price:+f.get('price'),cover_path:cover,teacher_image_path:ti,file_path:fp,file_name:pdfFile.name,status:'published'}); await audit('booklet','نشر ملزمة '+f.get('title')); await load(); renderBookletsAdmin(); }catch(e){ alert(e.message); } }
...
window.openLibraryBookletPdf=async function(orderId){
  ...
  if(!resolved){
    checkoutBox.innerHTML=`<h2>تعذر فتح الملزمة</h2><div class="empty">ملف PDF غير موجود في التخزين. احذف هذه الملزمة وارفع ملف PDF من جديد من لوحة المدير.</div><div class="row-actions no-print"><button class="secondary" onclick="closeCheckout()">إغلاق</button></div>`;
    return;
  }
  ...
  const pdfBlob = resolved.blob.type==='application/pdf'
    ? resolved.blob
    : new Blob([resolved.blob],{type:'application/pdf'});
  const objectUrl=URL.createObjectURL(pdfBlob);
  window.__alinPdfObjectUrl && URL.revokeObjectURL(window.__alinPdfObjectUrl);
  window.__alinPdfObjectUrl=objectUrl;

  checkoutBox.innerHTML=`<h2>مشاهدة الملزمة</h2><div class="pdf-viewer"><iframe id="pdfFrame" src="${objectUrl}#toolbar=0&navpanes=0&scrollbar=1"></iframe></div><div class="row-actions no-print"><button onclick="printPdfFrame()">طباعة</button><button class="secondary" onclick="closeCheckout()">إغلاق</button></div>`;
};
/* ===== end ALIN V50 ===== */

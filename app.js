...
async function saveSystemSettings(){ SUPABASE_URL=systemUrl.value.trim(); SUPABASE_ANON_KEY=systemKey.value.trim(); localStorage.setItem('ALIN_SUPABASE_URL',SUPABASE_URL); localStorage.setItem('ALIN_SUPABASE_ANON_KEY',SUPABASE_ANON_KEY); if(!initSupabase()){ systemMsg.textContent='أكمل بيانات النظام'; return; } systemMsg.textContent='تم حفظ إعدادات النظام'; await load(); renderSettingsAdmin(); }
...
async function load(){
  if(!initSupabase()){ renderAll(); return; }
  try{
...
function renderSettingsAdmin(){ adminContent.innerHTML=`<h2>إعدادات النظام</h2><p>إعدادات خاصة بالإدارة.</p><div class="form-grid"><input id="systemUrl" value="${SUPABASE_URL}" placeholder="رابط النظام"><input id="systemKey" type="password" value="${SUPABASE_ANON_KEY}" placeholder="مفتاح الربط"><button onclick="saveSystemSettings()">حفظ الإعدادات</button></div><div id="systemMsg"></div>`; }

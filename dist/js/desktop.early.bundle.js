// Alin desktop early bundle

// === core/config.js ===
/* ===== core/js/config.js ===== */
/* V106 central configuration. Keep secrets outside the repository. */
...
(function(){
  ...
  async function cloudInsert(table,row){
    const c=client(); if(!c||!navigator.onLine){queueMutation('insert',table,row);return {...row,_queued:true};}
    const {data,error}=await c.from(table).insert(row).select().single();
    if(error) throw error;
    return data;
  }
  async function cloudUpdate(table,values,match){
    const c=client(); if(!c||!navigator.onLine){queueMutation('update',table,values,match);return {_queued:true};}
  ...
  async function cloudDelete(table,match){
    const c=client(); if(!c||!navigator.onLine){queueMutation('delete',table,null,match);return {_queued:true};}
  ...
  async function flushQueue(){
    ...
    for(const item of q){
      ...
      }catch(e){ item.attempts=(item.attempts||0)+1; item.last_error=normalizeError(e); if(item.attempts<5) remain.push(item); }
    }
  ...
  async function loadCloudSnapshot(){
    ...
    await Promise.all(RC5_TABLES.map(async t=>{
      try{ rows[t]=await selectAll(t,{orderBy:['orders','notifications','audit','order_timeline'].includes(t)?'created_at':undefined,ascending:false}); }

...
for(const htmlName of ['store-desktop.html','store-mobile.html']){
  ...
  if(!html.includes('version-badge">v2.2.7'))failures.push(`version-badge:${htmlName}`);
}
...
for(const htmlName of ['store-desktop.html','store-mobile.html']){
  const html=fs.readFileSync(path.join(root,htmlName),'utf8');
  const positions=moduleFiles.map(rel=>html.indexOf(`./${rel}?v=2.2.7`));
...
for(const htmlName of ['store-desktop.html','store-mobile.html']){
  const html=fs.readFileSync(path.join(root,htmlName),'utf8');
  if(!html.includes('core/finance-runtime.js?v=2.2.7'))failures.push(`finance-script:${htmlName}`);
}
